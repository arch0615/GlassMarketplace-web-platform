...
import { Wrench, Eye, Droplets, HelpCircle, Upload, X, Loader2 } from 'lucide-react'
...
const SERVICE_OPTIONS = [
  { id: 'reparacion', label: 'Arreglo / Reparación', desc: 'Cambio de armazón, tornillos, soldaduras, ajustes', icon: Wrench, color: 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' },
  { id: 'lentes_contacto', label: 'Lentes de contacto', desc: 'Cotización de lentes de contacto (blandas o rígidas)', icon: Eye, color: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' },
  { id: 'liquidos_accesorios', label: 'Líquidos / Insumos', desc: 'Soluciones de limpieza, estuches, paños, accesorios', icon: Droplets, color: 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400' },
  { id: 'otro', label: 'Otro servicio', desc: 'Cualquier otra consulta óptica', icon: HelpCircle, color: 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400' },
]
...
export default function NuevaArregloPublica() {
  ...
  async function uploadPhotoAnon() {
    if (!photoFile) return null
    const fd = new FormData()
    fd.append('image', photoFile)
    const res = await fetch('/api/requests/upload-photo-public', { method: 'POST', body: fd })
    if (!res.ok) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      try {
        ...
        // ignore — backend will fall back to all opticas
      }

      const photoUrl = photoFile ? await uploadPhotoAnon() : null
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5 space-y-3">
        ...
        <textarea
          ...
          placeholder={
            activeOption?.id === 'reparacion'
              ...
              ? 'Ej: Necesito lentes de contacto blandas, esférica, -2.25 ambos ojos.'
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-slate-400 dark:text-slate-500">{MAX_DESC - description.length} restantes</span>
        </div>
      </Card>
      ...
      <Card className="p-5 space-y-3">
        <div>
          <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">Foto (opcional)</label>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
        ...
        {!photoPreview ? (
          ...
          >
            {compressing ? <Loader2 className="w-7 h-7 text-slate-400 animate-spin" /> : <Upload className="w-7 h-7 text-slate-400" />}
          ...
          <div className="relative">
            <img src={photoPreview} alt="Vista previa" className="w-full max-h-64 object-contain rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900" />
            <button type="button" onClick={clearPhoto} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-white dark:bg-slate-800 shadow flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-700">
              <X className="w-4 h-4 text-slate-500" />
            </button>
          </div>
        )}
        <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(e) => handleFile(e.target.files?.[0])} className="hidden" />
      </Card>
